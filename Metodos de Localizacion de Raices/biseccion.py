...
def biseccion(a, b, tolerancia=1e-5, tipo_error='absoluto', max_iter=100):
    ...
    while iteracion < max_iter:
        ...
        # Calcular error solo despues de la primera iteracion
        ...
        # Verificar si encontramos la raiz exacta
        if abs(f(c)) < 1e-10:
            print(f"\n[Biseccion] Raíz exacta encontrada: {c:.6f}")
            return c
        
        # Verificar criterio de parada despues de la primera iteracion
        if c_viejo is not None and error <= tolerancia:
            print(f"\n[Biseccion] Convergencia alcanzada:")
            print(f"Raíz aproximada: {c:.6f}")
    ...
    if c_viejo is not None:
        error = calcular_error(c, c_viejo, tipo_error)
    print(f"\n[Biseccion] Maximo de iteraciones alcanzado:")
